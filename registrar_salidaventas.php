...
function ajaxTipoPrecio(){
	var contenedor;
	contenedor=document.getElementById("divTipoPrecio");
	var tipoOrigen=document.getElementById("tipoOrigen").value;
	ajax=nuevoAjax();
	ajax.open("GET", "ajaxTipoPrecio2.php?tipoOrigen="+tipoOrigen,true);
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4) {
			contenedor.innerHTML = ajax.responseText;
		}
...
function menosCancelar() {
	cantidad_items--;
	console.log("FILA ELIMINAR: "+num);
	fi = document.getElementById('fiel');
...
{   
	...
	if(cantidadItems>0){
		...
		for(var i=1; i<=cantidadItems; i++){
			//itemValidar=parseFloat(document.getElementById("materiales"+i).value);
			if(document.getElementById("materiales"+i)!=null && document.getElementById("materiales"+i).value!=0){
				...
				alert("Debe seleccionar un item. Fila: "+i);
				return(false);
			}
		}
...
<input type="hidden" name="codConsumo" id="codConsumo" value="0">
...
<td>
	<?php
		$sql1="select codigo, nombre from tipos_origenpedido order by 1";
		$resp1=mysql_query($sql1);
		echo "<select name='tipoOrigen' class='texto' id='tipoOrigen' onChange='ajaxTipoPrecio(form1);'>";
...
<fieldset id="fiel" style="width:100%;border:0;">
	...
	<table id='pieNota' width='100%' border="0">
		...
		<tr>
			<td align='right' width='90%'>Descuento Bs.</td><td><input type='number' name='descuentoVenta' id='descuentoVenta' onChange='aplicarDescuento(form1);' value="0" readonly></td>
		</tr>
		<tr>
...
<div id="divboton" style="position: absolute; top:20px; left:920px;visibility:hidden; text-align:center; z-index:3">
	<a href="javascript:menosCancelar();"><img src="imagenes/cerrar4.png" height="45px" width="45px"></a>
</div>
...
<div id="divProfileData" style="background-color:#FFF; width:750px; height:350px; position:absolute; top:50px; left:170px; -webkit-border-radius: 20px; 	-moz-border-radius: 20px; visibility: hidden; z-index:2; overflow: auto;">
  	<div id="divProfileDetail" style="visibility:hidden; text-align:center">
		<table align='center'>
			...
			<tr>
			<td>
				<input type='text' name='itemNombreMaterial' id='itemNombreMaterial' size="35" class="textogranderojo" onkeypress="return pressEnter(event, this.form);">
			</td>
			<td>
				<input type='button' class='boton' value='Buscar' onClick="listaMateriales(this.form);">
				<input type='button' class='boton2' value='Cancelar' onClick="menosCancelar();">
			</td>
			</tr>
