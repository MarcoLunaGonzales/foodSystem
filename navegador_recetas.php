...
echo "<script language='Javascript'>
		function enviar_nav()
		{	location.href='registrar_preparacion.php';
		}
		function eliminar_nav(f)
		{
			...
			else
			{
				...
				{
					location.href='eliminar_preparaciones.php?datos='+datos+'';
				}
				else
				{
					return(false);
		...
		function editar_nav(f)
		{
			...
			else
			{
				...
				{
					location.href='editar_preparacion.php?cod_material='+j_ciclo+'';
				}
			}
		}
				function duplicar(f)
		{
			...
			else
			{
				...
				{
					location.href='duplicarProducto.php?cod_material='+j_ciclo+'&tipo=2';
				}
			}
		}
		function cambiar_vista(sel_vista, f)
		{
			...
			location.href='navegador_recetas.php?vista='+modo_vista+'';
		}
		</script>";
	...
	echo "<h1>Registro de Preparaciones</h1>";
	...
	$sql="select m.codigo_material, m.descripcion_material, m.estado, 
		...
		(select ta.nombre from tamanios ta where ta.codigo=m.cod_tamanio)
		from material_apoyo m
		where m.estado='1' and m.cod_tipomaterial in (3) order by m.descripcion_material";
	if($vista==1)
	{	$sql="select m.codigo_material, m.descripcion_material, m.estado, 
		(select e.nombre_grupo from grupos e where e.cod_grupo=m.cod_grupo), 
		(select t.nombre_tipomaterial from tipos_material t where t.cod_tipomaterial=m.cod_tipomaterial), 
		...
		(select ta.nombre from tamanios ta where ta.codigo=m.cod_tamanio)
		from material_apoyo m
		where m.estado='0' and m.cod_tipomaterial in (3) order by m.descripcion_material";
	}
	
	//echo $sql;
	$resp=mysql_query($sql);
	
	echo "<table align='center' class='texto'><tr><th>Ver Preparaciones:</th>
	...
	echo "<center><table border='0' class='textomini'><tr><th>Leyenda:</th><th>Preparaciones Retiradas</th><td bgcolor='#ff6666' width='30%'></td></tr></table></center><br>";
	...
	echo "<tr><th>Indice</th><th>&nbsp;</th><th>Nombre Preparacion</th><th>Descripcion</th>
		<th>Grupo</th><th>Tamano</th><th>Tipo</th><th>Precio Venta</th><th>&nbsp;</th><th>&nbsp;</th><th>&nbsp;</th></tr>";
	...
	while($dat=mysql_fetch_array($resp))
	{
		...
		$txtDetalle="<table class='texto'><tr><th>Insumo</th><th>Cant</th></tr>";
