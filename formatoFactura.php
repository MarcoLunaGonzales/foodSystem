...
$sqlNro="select count(*) from `salida_detalle_almacenes` s where s.`cod_salida_almacen`=$codigoVenta and s.estado=1";
$respNro=mysql_query($sqlNro);
$nroItems=mysql_result($respNro,0,0);

$tamanoLargo=250+($nroItems*3)-3;
...
$pdf->SetXY(0,$y+40);		$pdf->MultiCell(0,3,"Sr(es): $razonSocialCliente",0,"C");
$y=$y+8;
...
$sqlDetalle="select m.codigo_material, s.`cantidad_unitaria`, m.`descripcion_material`, s.`precio_unitario`, 
		...
		m.`codigo_material`=s.`cod_material` and s.`cod_salida_almacen`=$codigoVenta and s.estado=1";
...
while($datDetalle=mysql_fetch_array($respDetalle)){
	...
	$montoUnitF=number_format($montoUnit,2);
	
	$pdf->SetXY(6,$y+$yyy);		$pdf->MultiCell(45,3,"$nombreMat",0,"L");
	$pdf->SetXY(50,$y+$yyy+1);		$pdf->Cell(7,0,"$cantUnit",0,0,"R");
	$pdf->SetXY(60,$y+$yyy+1);		$pdf->Cell(10,0,"$montoUnitF",0,0,"R");
...
$montoTotalF=number_format($montoTotal,2);

$pdf->SetXY(40,$y+$yyy);		$pdf->Cell(0,0,"Total Venta:",0,0);
$pdf->SetXY(60,$y+$yyy);		$pdf->Cell(10,0,"$montoTotalF",0,0,"R");
$pdf->SetXY(40,$y+$yyy+4);		$pdf->Cell(0,0,"Descuento:",0,0);
$pdf->SetXY(60,$y+$yyy+4);		$pdf->Cell(10,0,"0",0,0,"R");
$pdf->SetXY(40,$y+$yyy+8);		$pdf->Cell(0,0,"Total Final:",0,0);
$pdf->SetXY(60,$y+$yyy+8);		$pdf->Cell(10,0,"$montoTotalF",0,0,"R");
...
$pdf->SetXY(6,$y+$yyy+11);		$pdf->MultiCell(0,3,"Son: $txtMonto"." ".$montoDecimal."/100 Bolivianos",0,"L");
